...
def find_hosts(net):
    ...
    # HierarchyTopo names hosts as 'Host1'..'Host12'; fallback to 'h1'..'h12'
    hosts = {}
    for i in range(1, 13):
        canonical = "Host%d" % i
        alias = "h%d" % i
...
def scenario_udp_jitter_loss(net, hosts, verbose=False):
    ...
    if verbose:
        print("Starting UDP server on h12")
    h12.cmd(server_cmd)
    time.sleep(0.5)
    ...
    if verbose:
        print("Running UDP client on h1...")
...
def scenario_first_vs_subsequent_ping(net, hosts, verbose=False):
    ...
    h9 = hosts.get("h9")
    result = {
        "scenario": "first_vs_subsequent",
        "first_packet_ms": None,
        "subsequent_avg_ms": None,
    }
    if not h1 or not h9:
        result["error"] = "hosts_missing"
        return result
...
def scenario_latency_during_convergence(net, hosts, verbose=False):
    ...
    h12 = hosts.get("h12")
    result = {
        "scenario": "convergence_latency_spike",
    ...
    p = h1.popen(
        "ping -c 30 -i 0.2 10.0.0.12", stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def save_results(
    results,
    output_dir="results/hierarchy/latency",
    algo_name="unknown",
    metric_name="latency",
):
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="End-to-End Latency Measurement Experiment (Hierarchy)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default="results/hierarchy/latency",
    )
    parser.add_argument(
