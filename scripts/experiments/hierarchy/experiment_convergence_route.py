...
def run_experiment(net, host1, host9_ip, controller_log_path=None):
    ...
    print("  Aksi: Tunggu 2 detik, lalu simulasikan Dist2 down dengan memutus semua linknya...")
    time.sleep(2)
...
def save_results_to_csv(results, output_dir='results/hierarchy/convergence', algo_name='unknown', metric_name='convergence'):
...
def save_per_scenario_csvs(results, output_dir='results/hierarchy/convergence', algo_name='unknown', metric_name='convergence'):
    os.makedirs(output_dir, exist_ok=True)
    paths = []
...
def main():
    ...
    parser.add_argument('--controller-cmd', default='python3 SPF/dijkstra_osken_controller.py --verbose',
                       help='Controller command to start')
    parser.add_argument('--algo-name', default=None,
                       help='Algorithm label for CSV filenames (e.g., dijkstra, astar)')
    parser.add_argument('--output-dir', default='results/hierarchy/convergence',
    ...
    try:
        ...
        # Save to CSV
        csv_path = save_results_to_csv(results, args.output_dir, algo_name=algo_name, metric_name=metric_name)
        save_per_scenario_csvs(results, args.output_dir, algo_name=algo_name, metric_name=metric_name)
