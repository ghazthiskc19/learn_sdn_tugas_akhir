...
def get_hop_count(net, src_host, dst_host, verbose=False):
    ...
    for sw in net.switches:
        ...
        if out and mac_src and mac_dst:
            if ('dl_src=%s' % mac_src.lower()) in out.lower() and ('dl_dst=%s' % mac_dst.lower()) in out.lower():
                counter += 1
                if verbose:
...
def run_progressive_distances(net, hosts, verbose=False):
    ...
    for src, dst in pairs:
        ...
        results.append({'test': 'progressive', 'src': src, 'dst': dst, 'hop_count': hop})
        if verbose:
...
def run_reroute_scenario(net, hosts, verbose=False):
    ...
    results.append({'test': 'reroute', 'phase': 'baseline', 'src': 'h1', 'dst': 'h9', 'hop_count': baseline})
    ...
    results.append({'test': 'reroute', 'phase': 'core_failure', 'src': 'h1', 'dst': 'h9', 'hop_count': core_fail})
    ...
    results.append({'test': 'reroute', 'phase': 'dist_failure', 'src': 'h1', 'dst': 'h9', 'hop_count': dist_fail})
...
def save_results(results, output_dir='results/hierarchy/hopcount', algo_name='unknown', metric_name='hopcount'):
...
def save_per_scenario_csvs(results, output_dir='results/hierarchy/hopcount', algo_name='unknown', metric_name='hopcount'):
    os.makedirs(output_dir, exist_ok=True)
    keys = set()
    for r in results:
        keys.update(r.keys())
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Hop Count Measurement Experiment (Hierarchy)')
    parser.add_argument('--algo-name', help='Algorithm label for CSV filenames (e.g., dijkstra, astar)', default=None)
    parser.add_argument('--output-dir', help='Output directory for results', default='results/hierarchy/hopcount')
