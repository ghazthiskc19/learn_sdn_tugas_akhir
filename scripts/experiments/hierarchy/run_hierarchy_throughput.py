#!/usr/bin/env python3
"""Hierarchy throughput benchmark for SPF controllers.

Runs the Hierarchy topology against BFS, A*, Bellman-Ford, and Dijkstra.
For each algorithm it runs a short warm-up ping sequence, then measures TCP
throughput using `iperf3` from Host1 (client) to Host4 (server) for 20s
with 1s interval reporting. Outputs per-second throughput samples to CSV.

This script uses `bandwidth_mbps` as the weight field for weighted
controllers (Dijkstra/A*/Bellman-Ford). BFS remains unweighted.
"""
...
def build_bandwidth_weights() -> Dict[str, Dict[str, float]]:
    """Return bandwidth map for the hierarchy topology.

    Keys are min:max switch ids as strings (e.g. "1:2"). Values are in Mbps.
    """
    links = {
        "1:2": {"bandwidth_mbps": 400},
        "1:3": {"bandwidth_mbps": 1000},
        "2:3": {"bandwidth_mbps": 1000},
        "4:5": {"bandwidth_mbps": 1000},
        "4:16": {"bandwidth_mbps": 50},
        "5:16": {"bandwidth_mbps": 500},
        "4:17": {"bandwidth_mbps": 500},
        "5:17": {"bandwidth_mbps": 500},
        "6:7": {"bandwidth_mbps": 500},
        "6:18": {"bandwidth_mbps": 500},
        "7:18": {"bandwidth_mbps": 500},
        "6:19": {"bandwidth_mbps": 1000},
        "7:19": {"bandwidth_mbps": 500},
        "1:4": {"bandwidth_mbps": 1000},
        "1:5": {"bandwidth_mbps": 400},
        "2:6": {"bandwidth_mbps": 1000},
        "2:7": {"bandwidth_mbps": 900},
    }
    # Ensure keys are strings and consistent
    return {"links": links}
...
def start_controller(
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix=f"hierarchy_thr_{algorithm}_"))
...
def run_single_algorithm(algorithm: str, weights_file: Path, ofp_port: int) -> List[ThroughputSample]:
    ...
    if pre_stop_log_text:
        # Look for flow install lines like in latency script
    ...
    if path:
        # Normalize and format path for human-readable output
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Hierarchy throughput benchmark")
    parser.add_argument(
        "--output",
        default=str(RESULTS_DIR / "hierarchy_throughput.csv"),
    ...
    with tempfile.TemporaryDirectory(prefix="hierarchy_weights_") as temp_dir:
        weights_file = Path(temp_dir) / "hierarchy_bw_weights.json"
