#!/usr/bin/env python3
"""Hierarchy latency benchmark for SPF controllers.

Runs the Hierarchy topology against BFS, A*, Bellman-Ford, and Dijkstra.
For each algorithm it captures 50 ICMP RTT samples from Host1 to Host4,
...
FLOW_INSTALL_RE = re.compile(r"\[FLOW-INSTALL\]\s+([0-9a-f:]{17})\s+→\s+([0-9a-f:]{17}):\s+(.+)")
...
def build_delay_weights() -> Dict[str, Dict[str, float]]:
    """Return the hierarchy delay map using the same switch pairs as the topology."""
    links = {
        "1:2": {"delay_ms": 50.0},
        "1:3": {"delay_ms": 0.5},
        "2:3": {"delay_ms": 0.5},
...
def format_path_labels(path: str) -> str:
    ...
    labels = []
    for node in nodes:
        labels.append(SWITCH_LABELS.get(node, f"s{node}"))
    return " -> ".join(labels)
...
def start_controller(algorithm: str, controller_path: Path, weights_file: Path) -> tuple[subprocess.Popen, Path]:
    temp_dir = Path(tempfile.mkdtemp(prefix=f"hierarchy_{algorithm}_"))
...
def run_single_algorithm(algorithm: str, delay_weights: Dict[str, Dict[str, float]], weights_file: Path) -> ExperimentResult:
    ...
    try:
        ...
        # Prime ARP / host learning with one short exchange.
        ...
        # Capture controller logs before Mininet teardown events mutate topology.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Hierarchy latency benchmark")
    parser.add_argument(
        "--output",
        default=str(RESULTS_DIR / "hierarchy_latency.csv"),
    ...
    with tempfile.TemporaryDirectory(prefix="hierarchy_weights_") as temp_dir:
        ...
        for algorithm in args.algorithms:
            result = run_single_algorithm(algorithm, build_delay_weights(), weights_file)
