...
def parse_iperf3_sender(output):
    ...
    res = {'transfer': None, 'bitrate': None, 'retransmits': None}
    # Try structured regex: transfer (MBytes/GBytes)  bitrate (Mbits/sec/Gbits/sec)  retrans
    m = re.search(r'([0-9]+(?:\.[0-9]+)?\s*(?:MBytes|GBytes))\s+([0-9]+(?:\.[0-9]+)?\s*(?:Mbits/sec|Gbits/sec))\s+(\d+)\s+.*sender', sender_line, flags=re.IGNORECASE)
    if not m:
        # Some iperf3 variants print without retrans column; try capturing transfer and bitrate
        m = re.search(r'([0-9]+(?:\.[0-9]+)?\s*(?:MBytes|GBytes))\s+([0-9]+(?:\.[0-9]+)?\s*(?:Mbits/sec|Gbits/sec))', sender_line, flags=re.IGNORECASE)
        if m:
    ...
    m_tr = re.search(r'([0-9]+(?:\.[0-9]+)?\s*(?:MBytes|GBytes))', sender_line, flags=re.IGNORECASE)
    m_br = re.search(r'([0-9]+(?:\.[0-9]+)?\s*(?:Mbits/sec|Gbits/sec))', sender_line, flags=re.IGNORECASE)
    m_re = re.search(r'(\d+)\s*(?:retransmits|retransmits|retrans|retries)?', sender_line, flags=re.IGNORECASE)
    if m_tr:
...
def scenario_throughput_baseline(net, hosts, verbose=False):
    ...
    h1 = hosts.get('h1')
    h9 = hosts.get('h9')
    result = {'scenario': 'baseline', 'transfer': None, 'bitrate': None, 'retransmits': None}
    if not h1 or not h9:
    ...
    result.update({'transfer': parsed.get('transfer'), 'bitrate': parsed.get('bitrate'), 'retransmits': parsed.get('retransmits')})
...
def scenario_throughput_multiflow(net, hosts, verbose=False):
    ...
    h10 = hosts.get('h10')
    h11 = hosts.get('h11')
    result = {'scenario': 'multiflow', 'transfer': None, 'bitrate': None, 'retransmits': None}
    ...
    p1 = h1.popen('iperf3 -c 10.0.0.9 -p 5201 -t 15', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    p2 = h2.popen('iperf3 -c 10.0.0.10 -p 5202 -t 15', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    p3 = h3.popen('iperf3 -c 10.0.0.11 -p 5203 -t 15', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def scenario_throughput_failover(net, hosts, verbose=False):
    ...
    h1 = hosts.get('h1')
    h9 = hosts.get('h9')
    result = {'scenario': 'failover', 'transfer': None, 'bitrate': None, 'retransmits': None}
    if not h1 or not h9:
        result['error'] = 'hosts_missing'
        return result
    ...
    p = h1.popen('iperf3 -c 10.0.0.9 -p 5204 -t 20', stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
def scenario_throughput_mss_compare(net, hosts, verbose=False):
    ...
    h1 = hosts.get('h1')
    h9 = hosts.get('h9')
    result = {'scenario': 'mss_compare', 'small_transfer': None, 'small_bitrate': None, 'small_retransmits': None,
              'large_transfer': None, 'large_bitrate': None, 'large_retransmits': None}
    if not h1 or not h9:
        result['error'] = 'hosts_missing'
        return result
...
def save_results(results, output_dir='results/hierarchy/throughput', algo_name='unknown', metric_name='throughput'):
...
def save_per_scenario_csvs(results, output_dir='results/hierarchy/throughput', algo_name='unknown', metric_name='throughput'):
    os.makedirs(output_dir, exist_ok=True)
    keys = set()
    for r in results:
        keys.update(r.keys())
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Throughput Measurement Experiment (Hierarchy)')
    ...
    parser.add_argument('--output-dir', help='Output directory for results', default='results/hierarchy/throughput')
    ...
    try:
        ...
        time.sleep(1)
        results.append(scenario_throughput_mss_compare(net, hosts, verbose=args.verbose))
