...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Plot mesh hop count and path structure for latency results."
	)
	parser.add_argument(
		"--input",
		type=Path,
	...
	parser.add_argument(
		"--output",
		type=Path,
		default=Path(__file__).parent / "mesh" / "mesh_hop_count.png",
...
def draw_hop_count_panel(ax: plt.Axes, summary: pd.DataFrame) -> None:
	...
	ax.set_title("Mesh Hop Count by Algorithm", fontsize=15, fontweight="bold")
...
def draw_path_panel(ax: plt.Axes, summary: pd.DataFrame) -> None:
	ax.set_title("Mesh Path Structure and Hop Count Relationship", fontsize=15, fontweight="bold")
	ax.set_xlim(0, 1)
...
def plot_hop_count(summary: pd.DataFrame, output_path: Path) -> None:
	...
	fig.suptitle(
		"Mesh Hop Count and Route Path Overview",
		fontsize=17,
		fontweight="bold",
		y=0.98,
	)
