...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Plot mesh latency without BFS as a single-panel line chart."
	)
	parser.add_argument(
		"--input",
		type=Path,
	...
	parser.add_argument(
		"--output",
		type=Path,
		default=Path(__file__).parent / "mesh_latency_lineplot_no_bfs.png",
...
def plot_latency(frame: pd.DataFrame, output_path: Path) -> None:
	...
	ax.set_title("Mesh Latency vs ICMP Sequence (No BFS)", fontsize=15, fontweight="bold")
